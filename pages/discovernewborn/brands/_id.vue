...
export default {
  ...
  methods: {
    ...
    async getProductsOfTradeMark(idTradeMark) {
      try {
        const { data } = await this.$api.product.getByTrademark(idTradeMark, NEWBORN);
    ...
    async getTradeMark(trademarkId) {
      ...
      let color = "";
      try {
        const { data } = await this.$api.trademark.getById(trademarkId, NEWBORN);
        if (data.status.code === "0000") {
