...
export default {
  ...
  methods: {
    ...
    async getToy() {
      try {
        const { data } = await this.$api.product.getById(this.idProduct);
    ...
    async getCardsByUserStatus(status) {
      ...
      const { data } = await this.$api.card.getByUserAndStatus(userId, status);
      if (data.status.code === "0000") {
    ...
    async createCard() {
      ...
      const { data } = await this.$api.card.create(userData);
      if (data.status.code === "0000") {
